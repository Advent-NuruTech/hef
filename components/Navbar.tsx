...
export default function Navbar() {
  ...
  const publicNavItems = [
    { href: "/", label: "Home" },
    { href: "/fundamental-principles", label: "Fundamental Principles" },
    { href: "/baptismal-vows", label: "Baptismal Vows" },
            ...
            <div className="hidden lg:flex items-center gap-6">
              {!user ? (
                <>
                  {publicNavItems.map((item) => (
                    <Link
                      key={item.href}
                      href={item.href}
                      className="px-3 py-2 rounded-lg hover:bg-gray-50 transition-all duration-200 font-medium text-gray-700 hover:scale-105"
                ...
                <div ref={dropdownRef} className="flex items-center gap-6">
                  ...
                  {/* Members, Messages, Avatar Dropdown */}
                  <Link href="/members" className="p-2 rounded-lg hover:bg-gray-50 transition-all duration-200 hover:scale-105" onClick={handleNavigation}>
                    <FaUsers className="text-xl text-gray-600 hover:text-blue-600 transition-colors" />
                  </Link>

                  <Link href="/messages" className="relative p-2 rounded-lg hover:bg-gray-50 transition-all duration-200 hover:scale-105" onClick={handleNavigation}>
                  ...
                  <div className="relative">
                    <button onClick={() => setOpen(!open)} className="flex items-center gap-2 p-1 rounded-full hover:bg-gray-50 transition-all duration-200">
                      <div className="relative w-10 h-10">
                        <Image
                    ...
                    {open && (
                      <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50 animate-in fade-in slide-in-from-top-2">
                        ...
                        <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors text-red-600 font-medium">
            ...
            <div className="lg:hidden flex items-center gap-3">
              {user && (
                <div className="relative w-8 h-8">
                  <Image src={userProfile.photoURL} alt={userProfile.displayName} fill className="rounded-full object-cover border border-gray-300" />
                </div>
              )}

              <button onClick={() => setSidebarOpen(true)} className="p-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition-colors duration-200">
